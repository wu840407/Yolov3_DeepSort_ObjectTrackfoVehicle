...
def savetxt(img,frame_num,save_path, bbox, identities=None, offset=(0,0)):
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
        ...
        with open(save_path[:save_path.rfind('.')] + '.txt', 'a') as file:
            file.write(('%s,'*2+'%g,'+'%g' + '\n') % (frame_num, label, (x1+x2)/2/img[2],(y1+y2)/2/img[3]))  # label format  
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_txt:  # Write to file
                savetxt(img.shape,frame_num,save_path, bbox_xyxy, identities)
